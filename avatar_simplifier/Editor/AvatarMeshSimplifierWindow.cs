...
namespace UnityMeshSimplifier.Editor
{
    ...
    /// </summary>
    public sealed class AvatarMeshSimplifierWindow : EditorWindow
    {
        ...
        private sealed class MeshEntry
        {
            ...
            {
                get { return Owner is SkinnedMeshRenderer; }
            }
        }
        ...
        private readonly List<MeshEntry> entries = new List<MeshEntry>();
        ...
        private string statusMessage = "Select a GameObject in the Hierarchy. Its supported child meshes are scanned automatically.";

        [MenuItem("Tools/Unity Mesh Simplifier/Avatar Mesh Simplifier")]
        private static void OpenWindow()
        {
            AvatarMeshSimplifierWindow window = GetWindow<AvatarMeshSimplifierWindow>();
            window.titleContent = new GUIContent("Avatar Simplifier");
        ...
        private void DrawSourceSection()
        {
            ...
            using (new EditorGUI.DisabledScope(isProcessing))
            {
                root = (GameObject)EditorGUILayout.ObjectField(
                    new GUIContent(
                        "Selected root",
                        isProcessing
                            ? "The root is locked while mesh simplification is running."
                            : "The current Unity Hierarchy selection is used as the scan root."),
                    root,
                    typeof(GameObject),
                    true);
            }
            ...
            includeMeshFilters = EditorGUILayout.ToggleLeft("Include MeshFilter meshes", includeMeshFilters);
        ...
        private void DrawSettingsSection()
        {
            ...
            if (targetInputMode == TargetInputMode.TriangleCount)
            {
                targetTriangleCount = Mathf.Max(
                    ...
                    EditorGUILayout.DelayedIntField(
                        new GUIContent("Target triangles", ""),
                        targetTriangleCount));
            }
            else
            {
                remainingPercentage = EditorGUILayout.Slider(
                    new GUIContent("Remaining percentage", ""),
                    remainingPercentage,
                    MinRemainingPercentage,
                    100f);
                EditorGUILayout.LabelField(
                    string.Format("Approximate reduction: {0:0.##}%", 100f - remainingPercentage),
                    EditorStyles.miniLabel);
            }

            geometryProtection = (GeometryProtectionLevel)EditorGUILayout.EnumPopup(
                new GUIContent("Geometry Protection Level", ""),
                geometryProtection);
            EditorGUILayout.LabelField(GetGeometryProtectionDescription(geometryProtection), EditorStyles.wordWrappedMiniLabel);

            if (geometryProtection == GeometryProtectionLevel.Stage4)
            {
                EditorGUILayout.HelpBox(
                    "Stage 4 is very computationally expensive and may take a while. Consider setting a Max time budget below so the simplifier can stop safely and save the best valid result reached within that time.",
                    MessageType.Warning);
            }
            ...
            targetPassCount = Mathf.Clamp(
                EditorGUILayout.DelayedIntField(
                    new GUIContent(
                        "Maximum redistribution passes",
                        "Each pass waits for all active meshes to finish. If the combined target is still unmet, the remaining reduction is redistributed and another pass begins."),
                    targetPassCount),
                1,
                32);
            EditorGUILayout.LabelField(
                "All meshes finish the current pass before redistribution starts. Intermediate passes stay in memory; only the final mesh state is created, saved, and assigned.",
            ...
            maximumSimplificationTimeSeconds = Mathf.Clamp(
                ...
                86400f);
            EditorGUILayout.LabelField(
                maximumSimplificationTimeSeconds > 0f
                    ? string.Format("Stops decimation after about {0:0.###} seconds, then saves and applies the safe partial result.", maximumSimplificationTimeSeconds)
                    : "Unlimited time budget.",
                EditorStyles.wordWrappedMiniLabel);

            int automaticWorkerCount = Mathf.Max(1, SystemInfo.processorCount);
            DrawOutputFolderField();
            cleanUnusedGeneratedMeshes = EditorGUILayout.ToggleLeft(
                new GUIContent(
                    "Automatically clean unused generated meshes",
                    ""),
                cleanUnusedGeneratedMeshes);
            EditorGUI.EndDisabledGroup();
        }

        private static string GetGeometryProtectionDescription(GeometryProtectionLevel level)
        {
            switch (level)
            {
                case GeometryProtectionLevel.Stage1:
                    return "Stage 1: Surface projection.";
                case GeometryProtectionLevel.Stage3:
                    return "Stage 3: Enhanced avatar hybrid (recommended).";
                case GeometryProtectionLevel.Stage4:
                    return "Stage 4: Reference accuracy, highest geometry protection.";
                default:
                    return "Stage 2: Avatar hybrid.";
            }
        }
        ...
        private void DrawOutputFolderField()
        {
            ...
            if (GUILayout.Button("Browse", GUILayout.Width(70f)))
            {
                ...
                if (!string.IsNullOrEmpty(absolute))
                {
                    ...
                    else if (normalized.StartsWith(projectRoot + "/", StringComparison.OrdinalIgnoreCase))
                    {
                        outputFolder = normalized.Substring(projectRoot.Length + 1);
                    }
                    else
                    {
                        EditorUtility.DisplayDialog("Invalid output folder", "Choose a folder inside this Unity project's Assets directory.", "OK");
                    }
                }
            }
        ...
        {
            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.LabelField(string.Format("Meshes ({0})", entries.Count), EditorStyles.boldLabel);
            GUILayout.FlexibleSpace();

            EditorGUI.BeginDisabledGroup(isProcessing || entries.Count == 0);
            if (GUILayout.Button("All", GUILayout.Width(45f)))
                SetAllSelections(true);
            if (GUILayout.Button("None", GUILayout.Width(45f)))
                SetAllSelections(false);
            EditorGUI.EndDisabledGroup();
            EditorGUILayout.EndHorizontal();

            Rect listRect = EditorGUILayout.BeginVertical(GUI.skin.box);
            if (entries.Count == 0)
            {
                EditorGUILayout.Space(12f);
                EditorGUILayout.LabelField("No supported meshes found.", EditorStyles.centeredGreyMiniLabel);
                EditorGUILayout.Space(12f);
            }
            else
            {
                for (int i = 0; i < entries.Count; i++)
                    DrawMeshEntry(entries[i], previewTargets);
            }
        ...
        private void DrawMeshEntry(MeshEntry entry, Dictionary<Mesh, long> previewTargets)
        {
            ...
            GUIContent title = new GUIContent(
                entry.SourceMesh != null ? entry.SourceMesh.name : "<missing mesh>",
                entry.Owner != null ? entry.Owner.name : string.Empty);
            EditorGUILayout.LabelField(title, EditorStyles.boldLabel);
            GUILayout.FlexibleSpace();
            EditorGUILayout.LabelField(entry.RendererKind, EditorStyles.miniLabel, GUILayout.Width(120f));
            EditorGUILayout.EndHorizontal();

            EditorGUILayout.LabelField(entry.HierarchyPath ?? string.Empty, EditorStyles.miniLabel);
            EditorGUILayout.LabelField(
                string.Format("{0:N0} vertices   {1:N0} triangles   {2} submeshes",
                    entry.VertexCount,
                    entry.TriangleCount,
            ...
            long target;
            if (!missing && entry.TriangleCount > 0 && previewTargets != null && previewTargets.TryGetValue(entry.SourceMesh, out target))
            {
                float targetPercent = (float)((double)target * 100.0 / entry.TriangleCount);
                EditorGUILayout.LabelField(
                    target >= entry.TriangleCount
                        ? string.Format("Requested target: unchanged ({0:N0} triangles)", entry.TriangleCount)
                        : string.Format("Requested target: {0:N0} triangles ({1:0.##}% remaining)", target, targetPercent),
                    EditorStyles.miniLabel);
            }
            ...
            if (entry.MissingBoneSlotCount > 0)
            {
                EditorGUILayout.HelpBox(
                    string.Format(
                        "{0} of {1} renderer bone slot(s) are missing. Influences using those slots will be removed; remaining influences are renormalized and orphaned vertices inherit the nearest connected valid skinning.",
        ...
        private static void DrawJobProgress(SimplificationJob job)
        {
            ...
            int requestedReduction = Math.Max(0, source - target);
            int completedReduction = Math.Max(0, source - current);
            bool simplificationFinished =
                progress.State == JobState.Finalizing ||
                progress.State == JobState.Completed;
        ...
        private void DrawActionSection(Dictionary<Mesh, long> previewTargets)
        {
            ...
            foreach (Mesh mesh in uniqueMeshes)
            {
                ...
                selectedTargetTriangles += previewTargets.TryGetValue(mesh, out target) ? target : sourceTriangles;
            }

            EditorGUILayout.LabelField(statusMessage, EditorStyles.wordWrappedMiniLabel);
            EditorGUILayout.LabelField(
                string.Format("Selected: {0} renderer(s), {1} unique mesh(es)", selectedEntryCount, uniqueMeshCount),
                EditorStyles.miniLabel);
            if (isProcessing && activeJobs != null && activeJobs.Count > 0)
                DrawOverallProgress(activeJobs);
            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.LabelField(
                string.Format("Unique mesh triangles: {0:N0} -> {1:N0}", selectedSourceTriangles, selectedTargetTriangles),
                EditorStyles.miniLabel);
            GUILayout.FlexibleSpace();

            if (isProcessing)
            {
                if (GUILayout.Button("Cancel", GUILayout.Width(100f)) && cancellationSource != null)
                {
                    statusMessage = "Cancellation requested. Current collapse pass will stop at its next safe checkpoint.";
                    cancellationSource.Cancel();
                }
            }
            else
            {
                using (new EditorGUI.DisabledScope(selectedEntryCount == 0))
                {
                    if (GUILayout.Button("Simplify Selected", GUILayout.Width(145f)))
        ...
        private void ScanHierarchy(bool selectAllNewEntries)
        {
            ...
            for (int i = 0; i < entries.Count; i++)
            {
                Component owner = entries[i].Owner;
                if (owner != null)
                    oldSelections[owner.GetInstanceID()] = entries[i].Selected;
            }
            ...
            statusMessage = string.Format("Found {0} supported renderer component(s).", entries.Count);
            Repaint();
        }
            ...
            bool selectAllNewEntries)
        {
            ...
            var entry = new MeshEntry
            {
                ...
                HierarchyPath = GetHierarchyPath(root.transform, owner.transform),
            ...
            try
            {
                ...
                statusMessage = string.Format(
                    ...
                    requestedUnusedMeshCleanup && cleanedUnusedMeshCount > 0
                        ...
                        : string.Empty);
                Debug.Log("[Avatar Mesh Simplifier] " + statusMessage, root);
            }
        ...
        private static int CleanupUnusedGeneratedMeshAssets(
            string folder,
            IList<string> protectedAssetPaths)
        {
            ...
            if (protectedAssetPaths != null)
            {
                for (int i = 0; i < protectedAssetPaths.Count; i++)
                {
                    ...
                    if (!string.IsNullOrEmpty(path))
                        protectedPaths.Add(path.Replace('\\', '/'));
                }
            }

            var candidates = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            string[] meshGuids = AssetDatabase.FindAssets("t:Mesh", new[] { normalizedFolder });
            for (int i = 0; i < meshGuids.Length; i++)
            {
                string path = AssetDatabase.GUIDToAssetPath(meshGuids[i]).Replace('\\', '/');
                if (!IsPathInsideFolder(path, normalizedFolder) ||
                    !path.EndsWith(".asset", StringComparison.OrdinalIgnoreCase) ||
                    Path.GetFileNameWithoutExtension(path).IndexOf(
                        "_Simplified",
            ...
            var usedPaths = new HashSet<string>(protectedPaths, StringComparer.OrdinalIgnoreCase);
            MarkLoadedMeshReferences(candidates, usedPaths);

            var openScenePaths = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            int openSceneCount = UnityEngine.SceneManagement.SceneManager.sceneCount;
            for (int sceneIndex = 0; sceneIndex < openSceneCount; sceneIndex++)
            {
                UnityEngine.SceneManagement.Scene scene =
                    UnityEngine.SceneManagement.SceneManager.GetSceneAt(sceneIndex);
                if (scene.IsValid() && scene.isLoaded && !string.IsNullOrEmpty(scene.path))
                    openScenePaths.Add(scene.path.Replace('\\', '/'));
            }
            ...
            for (int pathIndex = 0;
                 ...
                 pathIndex++)
            {
                string ownerPath = projectPaths[pathIndex].Replace('\\', '/');
                if (string.IsNullOrEmpty(ownerPath) ||
                    candidates.Contains(ownerPath) ||
                    openScenePaths.Contains(ownerPath) ||
